...
export class AnalyticsService {
  ...
  async getTeamWorkload(organizationId: string) {
    ...
    const workloads = employees.map((emp) => {
      ...
      const queuedTasks = emp.assignments.filter(
        (a) => a.task.status === 'PENDING',
      );
      const blockedTasks = emp.assignments.filter(
      ...
      const currentTaskName = activeCount > 0
      ...
      return {
        ...
        avatar: emp.name.split(' ').map((n: string) => n[0]).join('').toUpperCase().substring(0, 2),
        load: capacity,
